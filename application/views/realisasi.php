<div class="row">
      <div class="col-lg-12">
      	  <h2 class="page-header" align="center">REALISASI</h2>
      </div>
</div><!--/.row-->
		<div class="row">
			<div class="col-lg-12">
				<div class="panel panel-default">
					<div class="panel-heading"><a href="#" class="btn btn-primary" data-target="#dialog-barang" data-toggle="modal">Tambah</a>
					</div>
					
					<div class="panel-body">

						<table class="table table-bordered table-striped">
					    <th rowspan="2" style="background: darkorange; text-align: center;">SUMBER DANA</th>
					    <th colspan="3" style="background: darkorange; text-align: center;">ESTIMASI</th>
					    <th colspan="3" style="background: darkorange; text-align: center;">REALISASI</th>
					    <th rowspan="2" style="background: darkorange; text-align: center;">NO. NOTA</th>
					    <th rowspan="2" style="background: darkorange; text-align: center;">AKSI</th>
					   
							  	</tr>

							   <td style="background: darkorange; text-align: center;"><b>BANYAK</b></td>
							   <td style="background: darkorange; text-align: center;"><b>HARGA SATUAN</b></td>
							   <td style="background: darkorange; text-align: center;"><b>JUMLAH</b></td>
							   <td style="background: darkorange; text-align: center;"><b>BANYAK</b></td>
							   <td style="background: darkorange; text-align: center;"><b>HARGA SATUAN</b></td>
							   <td style="background: darkorange; text-align: center;"><b>JUMLAH</b></td>
							   </tr>

							  <tr>
							   <td>LEMBAGA MAHASISWA</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
							  <tr>
							   <td>KAS AMCC</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
						</table>
					</div>
				</div>
        	</div>
        </div> <!--End .TABEL-->

<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default">
			<div class="panel-heading"><a href="#" class="btn btn-primary" data-target="#dialog-barang" data-toggle="modal">Tambah</a>
			</div>
					
					<div class="panel-body">

						<table class="table table-bordered table-striped">
					    <th rowspan="2" style="background: darkorange; text-align: center;">SEKRETARIS</th>
					    <th colspan="3" style="background: darkorange; text-align: center;">ESTIMASI</th>
					    <th colspan="3" style="background: darkorange; text-align: center;">REALISASI</th>
					    <th rowspan="2" style="background: darkorange; text-align: center;">NO. NOTA</th>
					    <th rowspan="2" style="background: darkorange; text-align: center;">AKSI</th>
					    
							  	</tr>

							   <td style="background: darkorange; text-align: center;"><b>BANYAK</b></td>
							   <td style="background: darkorange; text-align: center;"><b>HARGA SATUAN</b></td>
							   <td style="background: darkorange; text-align: center;"><b>JUMLAH</b></td>
							   <td style="background: darkorange; text-align: center;"><b>BANYAK</b></td>
							   <td style="background: darkorange; text-align: center;"><b>HARGA SATUAN</b></td>
							   <td style="background: darkorange; text-align: center;"><b>JUMLAH</b></td>
							   </tr>

							  <tr>
							   <td>PROPOSAL DAN PENGGANDAAN</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
							  <tr>
							   <td>FOTOCOPY</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
							   <tr>
							   <td>SURAT MENYURAT</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
						</table>
					</div>
				</div>
        	</div>
        </div> <!--End .TABEL-->
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default">
			<div class="panel-heading"><a href="#" class="btn btn-primary" data-target="#dialog-barang" data-toggle="modal">Tambah</a>
			</div>
					
					<div class="panel-body">

						<table class="table table-bordered table-striped">
					    <th rowspan="2" style="background: darkorange; text-align: center;">KONSUMSI</th>
					    <th colspan="3" style="background: darkorange; text-align: center;">ESTIMASI</th>
					    <th colspan="3" style="background: darkorange; text-align: center;">REALISASI</th>
					    <th rowspan="2" style="background: darkorange; text-align: center;">NO. NOTA</th>
					    <th rowspan="2" style="background: darkorange; text-align: center;">AKSI</th>
					    
							  	</tr>

							   <td style="background: darkorange; text-align: center;"><b>BANYAK</b></td>
							   <td style="background: darkorange; text-align: center;"><b>HARGA SATUAN</b></td>
							   <td style="background: darkorange; text-align: center;"><b>JUMLAH</b></td>
							   <td style="background: darkorange; text-align: center;"><b>BANYAK</b></td>
							   <td style="background: darkorange; text-align: center;"><b>HARGA SATUAN</b></td>
							   <td style="background: darkorange; text-align: center;"><b>JUMLAH</b></td>
							   </tr>

							  <tr>
							   <td>MAKAN SIANG PESERTAN</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
							  <tr>
							   <td>MAKAN SIANG PANITIA</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
							   <tr>
							   <td>MKAN SIANG TAMU UNDANGAN</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
						</table>
					</div>
				</div>
        	</div>
        </div> <!--End .TABEL-->        
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default">
			<div class="panel-heading"><a href="#" class="btn btn-primary" data-target="#dialog-barang" data-toggle="modal">Tambah</a>
			</div>
					
					<div class="panel-body">

						<table class="table table-bordered table-striped">
					    <th rowspan="2" style="background: darkorange; text-align: center;">PUBLIKASI DAN DOKUMENTASI</th>
					    <th colspan="3" style="background: darkorange; text-align: center;">ESTIMASI</th>
					    <th colspan="3" style="background: darkorange; text-align: center;">REALISASI</th>
					    <th rowspan="2" style="background: darkorange; text-align: center;">NO. NOTA</th>
					    <th rowspan="2" style="background: darkorange; text-align: center;">AKSI</th>
							  	</tr>

							   <td style="background: darkorange; text-align: center;"><b>BANYAK</b></td>
							   <td style="background: darkorange; text-align: center;"><b>HARGA SATUAN</b></td>
							   <td style="background: darkorange; text-align: center;"><b>JUMLAH</b></td>
							   <td style="background: darkorange; text-align: center;"><b>BANYAK</b></td>
							   <td style="background: darkorange; text-align: center;"><b>HARGA SATUAN</b></td>
							   <td style="background: darkorange; text-align: center;"><b>JUMLAH</b></td>
							   </tr>

							  <tr>
							   <td>CETAK BACKDROP</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
							  <tr>
							   <td>CETAK BANNER</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
							   <tr>
							   <td>CETAK POSTER</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
						</table>
					</div>
				</div>
        	</div>
        </div> <!--End .TABEL--> 
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default">
			<div class="panel-heading"><a href="#" class="btn btn-primary" data-target="#dialog-barang" data-toggle="modal">Tambah</a>
			</div>
					
					<div class="panel-body">

						<table class="table table-bordered table-striped">
					    <th rowspan="2" style="background: darkorange; text-align: center;">PERLENGKAPAN</th>
					    <th colspan="3" style="background: darkorange; text-align: center;">ESTIMASI</th>
					    <th colspan="3" style="background: darkorange; text-align: center;">REALISASI</th>
					    <th rowspan="2" style="background: darkorange; text-align: center;">NO. NOTA</th>
					    <th rowspan="2" style="background: darkorange; text-align: center;">AKSI</th>
							  	</tr>

							   <td style="background: darkorange; text-align: center;"><b>BANYAK</b></td>
							   <td style="background: darkorange; text-align: center;"><b>HARGA SATUAN</b></td>
							   <td style="background: darkorange; text-align: center;"><b>JUMLAH</b></td>
							   <td style="background: darkorange; text-align: center;"><b>BANYAK</b></td>
							   <td style="background: darkorange; text-align: center;"><b>HARGA SATUAN</b></td>
							   <td style="background: darkorange; text-align: center;"><b>JUMLAH</b></td>
							   </tr>

							  <tr>
							   <td>SEWA SOUND SYSTEM</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
							  <tr>
							   <td>SEWA HT</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
							   <tr>
							   <td>SURAT MENYURAT</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
						</table>
					</div>
				</div>
        	</div>
        </div> <!--End .TABEL--> 
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default">
			<div class="panel-heading"><a href="#" class="btn btn-primary" data-target="#dialog-barang" data-toggle="modal">Tambah</a>
			</div>
					
					<div class="panel-body">

						<table class="table table-bordered table-striped">
					    <th rowspan="2" style="background: darkorange; text-align: center;">ACARA</th>
					    <th colspan="3" style="background: darkorange; text-align: center;">ESTIMASI</th>
					    <th colspan="3" style="background: darkorange; text-align: center;">REALISASI</th>
					    <th rowspan="2" style="background: darkorange; text-align: center;">NO. NOTA</th>
					    <th rowspan="2" style="background: darkorange; text-align: center;">AKSI</th>
							  	</tr>

							   <td style="background: darkorange; text-align: center;"><b>BANYAK</b></td>
							   <td style="background: darkorange; text-align: center;"><b>HARGA SATUAN</b></td>
							   <td style="background: darkorange; text-align: center;"><b>JUMLAH</b></td>
							   <td style="background: darkorange; text-align: center;"><b>BANYAK</b></td>
							   <td style="background: darkorange; text-align: center;"><b>HARGA SATUAN</b></td>
							   <td style="background: darkorange; text-align: center;"><b>JUMLAH</b></td>
							   </tr>

							  <tr>
							   <td>FEE PEMBICARA</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
							  <tr>
							   <td>FEE PENGHIBUR ACARA</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
							   <tr>
							   <td>FOTOCOPY</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
						</table>
					</div>
				</div>
        	</div>
        </div> <!--End .TABEL--> 
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default">
			<div class="panel-heading"><a href="#" class="btn btn-primary" data-target="#dialog-barang" data-toggle="modal">Tambah</a>
			</div>
					
					<div class="panel-body">

						<table class="table table-bordered table-striped">
					    <th rowspan="2" style="background: darkorange; text-align: center;">HUMAS</th>
					    <th colspan="3" style="background: darkorange; text-align: center;">ESTIMASI</th>
					    <th colspan="3" style="background: darkorange; text-align: center;">REALISASI</th>
					    <th rowspan="2" style="background: darkorange; text-align: center;">AKSI</th>
							  	</tr>

							   <td style="background: darkorange; text-align: center;"><b>BANYAK</b></td>
							   <td style="background: darkorange; text-align: center;"><b>HARGA SATUAN</b></td>
							   <td style="background: darkorange; text-align: center;"><b>JUMLAH</b></td>
							   <td style="background: darkorange; text-align: center;"><b>BANYAK</b></td>
							   <td style="background: darkorange; text-align: center;"><b>HARGA SATUAN</b></td>
							   <td style="background: darkorange; text-align: center;"><b>JUMLAH</b></td>
							   </tr>

							  <tr>
							   <td>BIAYA TRANSPORTASI</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							  </tr>
							  <tr>
							   <td>PULSA</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
						</table>
					</div>
				</div>
        	</div>
        </div> <!--End .TABEL-->  
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default">
			<div class="panel-heading"><a href="#" class="btn btn-primary" data-target="#dialog-barang" data-toggle="modal">Tambah</a>
			</div>
					
					<div class="panel-body">

						<table class="table table-bordered table-striped">
					    <th rowspan="2" style="background: darkorange; text-align: center;">KESEKRETARIATAN</th>
					    <th colspan="3" style="background: darkorange; text-align: center;">ESTIMASI</th>
					    <th colspan="3" style="background: darkorange; text-align: center;">REALISASI</th>
					    <th rowspan="2" style="background: darkorange; text-align: center;">NO. NOTA</th>
					    <th rowspan="2" style="background: darkorange; text-align: center;">AKSI</th>
							  	</tr>

							   <td style="background: darkorange; text-align: center;"><b>BANYAK</b></td>
							   <td style="background: darkorange; text-align: center;"><b>HARGA SATUAN</b></td>
							   <td style="background: darkorange; text-align: center;"><b>JUMLAH</b></td>
							   <td style="background: darkorange; text-align: center;"><b>BANYAK</b></td>
							   <td style="background: darkorange; text-align: center;"><b>HARGA SATUAN</b></td>
							   <td style="background: darkorange; text-align: center;"><b>JUMLAH</b></td>
							   </tr>

							  <tr>
							   <td>FOTOCOPY</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
							  <tr>
							   <td>CETAK SERTIFIKAT</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
							   <tr>
							   <td>PRINT WARNA</td>
							   <td></td>
							   <td></td>
							   <td>2000000</td>
							   <td></td>
							   <th></th>
							   <td>2000000</td>
							   <td></td>
							   <td></td>
							  </tr>
						</table>
					</div>
				</div>
        	</div>
        </div> <!--End .TABEL-->                                   